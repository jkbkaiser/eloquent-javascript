...
SkillShareServer.prototype.updated = function () {
  this.version++;
  writeFile("talks.json", JSON.stringify(this.talks), function (err) {
    if (err) {
...
readFile("talks.json", "utf-8", (err, data) => {
  if (err) {
    new SkillShareServer({}).start(8000);
  } else {
    let initialTalks = JSON.parse(data);
    new SkillShareServer(initialTalks).start(8000);
  }
});
